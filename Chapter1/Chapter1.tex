...
but here they are not discussed) \citeref{Kopp2009}.  Given that the majority of dark matter must be
...
Here the standard halo model (SHM) is assumed,
though there is not a consensus on whether the DM velocity distribution can be described as Maxwell-Boltzmann
\citeref{Diemand2004, Kuhlen2009}.  For WIMP searches most experiments
...
\sigma^{\mathrm{SI}}^{(0)} = \sigma_{\mathrm{p}} \frac{\mu_{\mathrm{A}}^{2}}{\mu_{\mathrm{p}}^{2}} \frac{\big[ Z f^{\mathrm{p}} +
(A - Z) f^{\mathrm{n}} \big]^{2}}{(f^{\mathrm{p}})^{2}}
\end{equation}

\noindent where $\sigma_{\mathrm{p}}$ is the cross section of a proton Z is the number of protons and $\mu_{\mathrm{p}}$ is
the WIMP-nucleon reduced mass. $f^{\mathrm{p/n}}$ is the coupling strength for protons and neutrons, which are assumed to be
equivalent (see \citeref{Yaguna2017} for $f^{\mathrm{p}} \neq f^{\mathrm{n}}$).  Substituting \eqnref{eq:sigma_si} into
\eqnref{eq:diff_sigma_si} gives a spin-independent differential cross-section of

\begin{equation}
\frac{d \sigma_{\mathrm{SI}}}{dE} = \frac{m_{\mathrm{A}} \sigma_{\mathrm{p}}}{2 \mu_{\mathrm{p}}^{2} v^{2}}
 A^{2} \big| F(E) \big |^{2}
\end{equation}
...
N ( m_{\chi}, \sigma_{p}) = T \times M \times \int_{E_{\mathrm{min}}}^{E_{\mathrm{max}}} \frac{dR}{dE} dE
\end{equation}
...
almost always carrying 1/2 spin \citeref{Ellis1991, PICASSO2012}.
...
has been received pushback in \citeref{Barbea2014, Klinger2015}).
...
background assumptions showed that this preference is well below 1.7$\sigma$ \citeref{Aalseth2014, Davis2014}.  Another
