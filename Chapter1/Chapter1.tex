...
where ds is the distance traversed in space-time, $a(t)$ is the scale factor ($a=1$ today), and $k$ is the curvature of the
...
with $\Omega = \sum \Omega_{i}$ where $\Omega_{i}$ is a component of the universe.  Current measurements give density
...
that produces an increase in brightness when the source and lens are close to one another.  This has given astronomers
...
we can identify the correction cosmological makeup.  To do this we make use of

\begin{equation}
\delta T = \sum\limits_{l=0}^{\infty} \sum\limits_{m=-l}^{l} a_{lm}Y_{lm}(\theta, \phi)
\end{equation}

where $\delta T = T(\theta, \phi) - \langle T \rangle$ for a given $\theta , \phi$
...
where $P_{l}$ are the Legendre polynomials and $C_{l} = \frac{1}{2l + 1} \sum\limits_{m=-l}^{l} a_{lm}$.  The
...
where $h$ is the Hubble Constant divided by 100 and $\langle \sigma_{\mathrm{ann}} v \rangle$ is
...
for dark matter.
 ...
 %========
\subsection{Hot vs. Cold} \label{subsec:hot_vs_cold}
...
where $\rho_{0}$ is the local dark matter density, $m_{\chi}$ is the mass of a
