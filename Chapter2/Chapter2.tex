...
they also transfer energy as heat, which cannot be measured in time projection chambers (\secref{sec:tpcs}).\footnote{To date heat has not
been observed in electronic recoils.}  The process is shown in \figref{fig:interaction_channels}.  This chapter only considers interactions
...
Xe$^{+}$ (details of recombination are discussed in \secref{subsec:recombination}).  The process of recombination is shown in
\eqnref{eq:recomb} where $Q$ represents heat from the de-excitation of the doubly excited xenon atom.
...
The fraction of electrons and ions that recombine depends on whether an electric field exists at the interaction
...
only a fraction occurs in the expected window).  Electrons that escape recombination are discussed in \secref{subsec:secondary}.

\eqnref{eq:recomb} shows that excitons result from electron-ion recombination in addition to xenon atoms that are initially
...
\noindent for some fraction of \ce{Xe^*}.  The \electron is freed with kinetic energy
...
\ce{Xe^+}, so the electron-ion pair quickly recombines.  The result is a single exciton instead of the original two, with the rest of the
...
$24 \pm 1\ \mathrm{ns}$,
respectively \citeref{Mock2014}.  Singlet-to-triplet ratios have been measured to vary across recoil type and are shown in
\tabref{tab:singlet_to_triplet}.  It has been proposed that singlets which undergo superelastic collisions with thermal electrons may form
triplets \citeref{Hitachi1983}.  This hypothesis is supported by observations of singlet quenching in the gaseous state \citeref{Lorents1976}
and slower
recombination amongst low linear energy transfer tracks (\secref{subsec:stopping_power}), which allows more time for superelastic
collisions.
...
\noindent where the last two terms on the right side equation recombine according to \eqnref{eq:recomb_dimer_ionized}.  Like biexcitonic
...
\noindent where the photon has a mean wavelength of 178 nm.  Thus, ignoring quenching \eqnref{eq:recomb,eq:deexcite} show that for each
...
resolved in our detector, which records in 10 ns intervals.\footnote{While xenon is advantageous as described in \secref{sec:properties}, one
advantage of liquid argon is the singlet lifetime of ${<}\, 6.2\ \mathrm{ns}$ is much shorter than that of the triplet,
$1.30 \pm 0.06\ \mathrm{\mu s}$ \citeref{Heindl2011}.}  These photons produce the first of two observables in time projection chambers,
...
\begin{table}[t]
 ...
 \citeref{Takiya2016, Kubota1978} are converted from the fraction of all excimers that are singlets.}
...
$v_d$.  Larger electric fields have lower recombination as the electromagnetic force from $E$ opposes that of the \ce{Xe^+}, leading to small
...
\citeref{Miller1968}.  As $E$ increases $v_{d} \propto E^{1/2}$ until approximately $10^4\ \mathrm{V\ cm^{-1}}$ where it flattens at
...
scintillation are found in \secref{subsec:tpcs_working_principle}.
...
Birk's saturation law explains biexcitonic quenching and the Penning process (\secref{subsec:primary}), which play major roles in nuclear
