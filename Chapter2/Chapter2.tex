...
it has been observed that two excitons may collide and free an electron as shown in \eqnref{eq:biexitonic}.
...
\vd and $E$ is given in \tabref{tab:drif_velocity} and shown in \figref{fig:drift_velocity}.
...
\ce{^{222}Rn} presents a similar problem.  As a daughter of the \ce{^{238}U} decay chain it emanates from the detector materials and distributes
...
be small, and the photodetectors themselves can have less than unity detection efficiency (see \secref{subsec:tpcs_pmts} for details on
PMTs).  To increase the intensity of light that reaches the photodetectors, and thereby improve the likelihood that an event is
...
passes through the window of the PMT will most often hit the photocathode, freeing a photoelectron via the photoelectric effect.  The
probability that an incident photon will eject a photoelectron, or the quantum efficiency (QE), is only approximately one third.  This
makes the reflectivity of the TPC wall material critical (\secref{tpcs_tpc}).  The photoelectron then passes through the focusing
...
lead to an under-amplified response as the cascade effect will effectively occur with one less dynode.  Wavelengths are 178, 128, and
78 nm for Xe, Ar, and Ne.

Details of the PMTs used in XENON1T can be found in \secref{}.

The sides of the chamber consist of a material that reflects the wavelength of the scintillation emitted inside the TPC.  Just outside
is a field cage: a series of rings stacked parallel to the meshes and connected via resistors.  These help maintain drift field uniformity
between the cathode and gate.

Below the cathode and above the anode are photomultiplier tubes.
