...
Liquid xenon (LXe) direct detection experiments have led the field for spin-independent WIMPs with masses $\gtrsim 20$ for roughly
a decade.  Even over liquid argon (LAr) the LXe results have been paved the way to new limits, surpassing on the way only those of other
LXe experiments.
...
Xenon is a noble gas with an atomic number of 54 with and a mean mass of 131.293 g mol$^{-1}$.  It is the heaviest non-radioactive noble
...
The naturally occurring Xe isotopes are listed in \tabref{tab:xe_isotopes}.  \ce{^{136}Xe}, with a fractional percentage of 8.8573\%,
has been measured to undergo double beta decay with a half-life of $> 2.4 \times 10^{21}\ \mathrm{y}$.  So while naturally
occurring xenon can be radioactive, the process is extremely rare.
...
lifetime (\secref{subsec:electron_lifetimes_measurement_gammas}).
...
which gives it sensitivity to spin-dependent interactions.  Its light and charge yields
(\secref{subsubsec:er_nr_calibrations_results_ly_qy}) are the highest among noble gases.  Finally, the amount of
...
\begin{table}
 ...
 observed but are predicted.  Half-life and decay information is taken from \citeref{Singh2007, Barros2014}.}
...
$\mathrm{^{129m}Xe}$ & 11/2 & 8.88 d & $39.6 + 196.6\ \mathrm{keV}\ \gamma$ & ground state \\
$\mathrm{^{131m}Xe}$ & 11/2 & 11.93 d & $163.9\ \mathrm{keV}\ \gamma$ & ground state \\
\ce{^{133}Xe} & 3/2 & 5.24 d & $\beta^-$ & \ce{^{133}Cs} \\
$\mathrm{^{133m}Xe}$ & 11/2 & 2.19 d & $233.2\ \mathrm{keV}\ \gamma$ & ground state \\
...
When a particle scatters off a xenon atom it can excite or ionize its valence electrons.  Here we refer to excited atoms Xe$^{*}$ as
...
If
an electric field is applied some $e^{-}$ will be pulled away from the interaction, lowering recombination and primary
scintillation but creating secondary scintillation (\secref{subsec:secondary}).  The extent depends on the strength of the electric
field.  However, even if no field is applied there will not be
100\% recombination as some of the freed $e^{-}$ will escape the electromagnetic pull of their parent ions.
...
\noindent $\gamma$ is the average de-excitation energy that carries a wavelength of 178 nm.  We can see that the number of photons is
anti-correlated with the number of electrons, since for each \electron that
recombines with xenon a single \gammaray is emitted.  The lifetimes for the singlet and triplet
excimers are $3.1 \pm 0.7$ ns and $24 \pm 1$ ns, respectively \citeref{Mock2014}.  The single-to-triplet ratios for
various interaction types are shown in \tabref{tab:singlet_to_triplet}.  An important method for detecting WIMPS is electronic
recoil-nuclear recoil
discrimination, and we see that nuclear recoils result in a substantially higher fraction of singlet states.  Unfortunately the lifetimes
...
where $\mu$ is the electron mobility.  For this energy region $\mu \sim 2200\ \mathrm{cm^{2}\ V^{-2}\ s^{-1}}$ \citeref{Miller1968}.  As
...
\caption{Electronic (top, $\gamma$) and nuclear (bottom, n) recoil tracks for a 20 keV scatter.  Excitons (red circles), ions (blue), and
...
the ratio is roughly equal.  Heat is only relevant in NRs and leads to a quenching of signal.  The electronic recoil has a path of
...
$\rho_{\mathrm{LXe}}$ gives an electron stopping power of $0.65 \mdash 30\ \mathrm{keV\ \mu m^{-1}}$ for \electron and
${\sim} 20 \mdash 900\ \mathrm{keV\ \mu m^{-1}}$ for $\alpha$ and protons.

We can see that in
the dark matter search region ($1 \mdash 100$ keV) $e^{-}$ decrease by a factor of ${\sim} 10$.  Multiplying by the density of LXe we get
the
electronic stopping power to be ${\sim} 0.65 \mdash 30\ \mathrm{keV\ \mu m^{-1}}$.  \alphadecays are typically $> 5.5$ MeV and thus have a
stopping
power of ${\sim} 600 \mdash 2000\ \mathrm{keV\ \mu m^{-1}}$.  We see that particle tracks are $\mathcal{O}(10)\ \mathrm{\mu m}$, so
particles that originate at or outside the surface of the detector will never make it into the FV with any reasonable probability.

\begin{figure}[t]
\includegraphics[width=0.8\textwidth]{stopping_power}
\caption{Electron mass stopping power for $\alpha$, $e^{-}$, and protons.  Data from \citeref{Berger2018a}}
...
$\varsigma E = 0.84\ \mathrm{kV\ cm^{-1}}$.  Curves are best fit for box model \citeref{Thomas1987}.}
\label{fig:ti_recomb}
\end{figure}
...
Law (originally developed for foro organic scintillators, \citeref{Birks1951, Birks1964}) derivation for liquid noble gases yields
...
\secref{subsec:primary}.  The process is
...
to form one excited
state and one ground state, which results in an additional quenching \citeref{Mei2008}.  In $\beta$ and $\gamma$ interactions the energy
...
while the latter can reach tens of GeV \citeref{Aprile2016a}.  Neutrons have a mean free path on the order of tens of cm, which makes
...
and $W = 13.7 \pm 0.2\ \mathrm{eV}$ \citeref{Dahl2009} is the energy needed to produce a single quantum.  This can be broken up into
...
Poisson distribution ($F = 1$).  Several experiments have attempted to measure $F$ for LXe \citeref{Doke1976, Seguinot1995} with an
...
but measurements today disagree \citeref{Doke2002, Aprile2007}.  Currently $N_{\mathrm{ex}} / N_{\mathrm{ion}}$ is expected to be
...
scale (discussed in \secref{subsubsec:tpcs_signals_energy}).  Unlike ERs, $N_{\mathrm{ex}} / N_{\mathrm{i}} \sim 1$
\citeref{Sorensen2011, Angle2011} due to higher density tracks.
