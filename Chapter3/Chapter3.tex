...
$\mu_{b}^{\mathrm{S1}}$ and standard deviation to be the smearing $\sigma_{s}^{\mathrm{S1}}$ for each bin.  This is applied in the fast MC via
\eqnref{eq:er_nr_calibrations_parameter_determ_det_phys_s1_bias_smear}.

\begin{equation}
\mathrm{S1} = n_{\mathrm{pe,S1}} \Big( 1 + \mathrm{N} \big( \mu = \mu_{b}^{\mathrm{S1}},\ \sigma^2 = (\sigma_{s}^{\mathrm{S1}})^2 \big)
\Big)
...
\begin{equation}
\tau_{e}^{\mathrm{true}} = \tau_{e}^{\mathrm{rec}} \bigg( 1 + \mathrm{N} \big( \mu = \tau_{e}^{\mathrm{rec}},\ \sigma^2 = 0.02^2 \big)
\bigg)
\end{equation}

\noindent where $\tau_{e}^{mathrm{rec}}$ and $\tau_{e}^{\mathrm{true}}$ refer to the reconstructed (measured) and true electron
lifetimes.  The true and reconstructed electron loss probabilities are given in
...
\begin{equation}
\mathrm{S2_b} = n_{\mathrm{pe,S2_b}} \Big( 1 + \mathrm{N}(\mu = \mu_{b}^{\mathrm{S2_b}},\ \sigma^2 = (\sigma_{s}^{\mathrm{S2_b}})^2) \Big)
\label{eq:er_nr_calibrations_parameter_determ_det_phys_s2_bias_smear}
\end{equation}

\noindent where $\mu_{b}^{\mathrm{S2_b}}$ and $\sigma_{s}^{\mathrm{S2_b}}$ are the \stwob bias and smearing,
...
may differ from that of just the bottom, to save memory we apply $\mu_{b}^{\mathrm{S2_b}}$ and $\sigma_{s}^{\mathrm{S2_b}}$.  Any
difference is too small to be relevant and can be disregarded.  $\mu_{b}^{\mathrm{S2_b}}$ and $\sigma_{s}^{\mathrm{S2_b}}$ are shown in
the right panel of \figref{fig:er_nr_calibrations_parameter_determ_det_phys_bias_smear}.
...
\begin{tabular}{ccccc}
\hline
Parameter & Initial & Prior & Posterior & Units & Note \\
\hline
$F$ & $0.0059$ & fixed & $-$ &  & Fano factor \\
$\eta$ & $0.933$ & fixed & $-$ &  & extraction efficiency \\
$\sigma_G$ & $0.250$ & fixed &  & $-$ & gas gain resolution \\
$v_d$ & $0.134$ & fixed & $-$ & $\mathrm{mm\ \mu s^{-1}}$ & drift velocity \\
$f_{\mathrm{aft}}$ & $0.627$ & fixed & $-$ &  & \\
$F_{\mathrm{E}}$ & $82$ & fixed & $-$ & $\mathrm{kV\ cm^{-1}$ & \\
$W$ & $\mu = 0.0137,\ \sigma = 0.0002$ & norm &  & keV & \\
