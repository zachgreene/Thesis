...
runs - for spin-independent WIMPs.  It begins with a description of the XENON1T experiment (\secref{sec:xenon1t_detector}), followed by
...
efficiency
(QE), or the ratio of photoelectrons (electrons ejected from the photocathode) divided by number of incident photons.  The R11410 window
...
To differentiate electronic from nuclear recoils calibrations were performed.  \ce{^{220}Rn} calibrations
were performed for electronic recoils seven times (one in SR0 and seven in SR1), while \ce{^{241}AmBe} and a deuterium-deuterium (D-D)
neutron generator (NG) were used for nuclear recoils and positioned in the water tank outside the cryostat.  \ambe calibrations were done
in both science runs and a NG calibration was performed in SR1.

Having reliable probabilities of different origins for an event is essential for a rare event search.  Fitting the electronic and nuclear
recoil bands (\chapref{chap:er_nr_calibrations}) creates probability density functions (PDFs) for our largest background and signal
regions.  A WIMP mass-dependent PDF is also generated from the signal model.\footnote{For this analysis the signal is a spin-independent
WIMP but other analyses investigate alternative models
\citeref{Aprile2013b, Aprile2014, Aprile2015a, Aprile2017a, Aprile2017g, Aprile2017h}.}  In
addition, PDFs from detector materials events (\secref{subsec:backgrounds_detector_materials}) and accidental coincidence
(\secref{subsec:backgrounds_ac}) are included.  The data is compared
to the total PDF for each WIMP mass and cross section to see how well it agrees with the signal model
...
fitting (\chapref{chap:er_nr_calibrations}).  While the 1.3 t fiducial mass is used for this analysis, the
acceptances are calculated using the 1 t mass from the band fitting though differences are expected to be small.
...
and S2 cuts in blue, total acceptance across all \cstwob in black, and total acceptance of the reference region in pink.  Dashed and solid
acceptance lines correspond to SR0 and SR1 acceptances,
...
The energy ROI is selected by requiring cS1 threshold cut $3 \leq \mathrm{cS1} \leq 70\ \mathrm{PE}$ (blue in, corresponding to roughly
...
cut acceptances.  Events inside the fiducial volume that are reconstructed outside - or vice versa - are expected to have a negligible
effect so are not included.  \figref{fig:dark_matter_results_acceptances} shows acceptances for 1) processor (``Detection''), 2) processor
and S1 and S2 cuts (``Selection''), 3) total for SR0 and SR1 across all $\mathrm{cS2_b}$, and 4) inside the $-2\sigma$ to median of the
nuclear recoil band.
