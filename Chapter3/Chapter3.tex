...
then detailed (\secref{sec:er_nr_calibrations}) and the finally dark matter search results (\secref{sec:dark_matter_results}) are
...
optimize the resolution of position reconstruction in $r$ (\secref{subsec:det_char_position_reconstruction}).  The 121 in the bottom array
...
top.  The difference in arrangement is strategic.  Due to liquid xenon's relatively large dielectric constant (1.95) an photons
originating inside the liquid may
...
A photoelectron is guided by a focusing electrode disk to the first of $N$ dynodes.  A voltage is applied to the photocathode and dynodes,
...
free some \electron in the first stage of an electron avalanche.  The electron avalanche will pass from dynode to dynode, rapidly
growing with each until it reaches the anode and is forwarded to the data acquisition.
...
meshes decreased stress on the phototubes and is expected to extend their life.  The finished arrays are shown in
\figref{fig:xenon1t_pmt_array}.
...
has a very low \ce{^{60}Co} concentration.  Finally, to insulate
the connections to each dynode the stem is ceramic.

The original R11410 version used ceramic as a dynode insulator, standard Kovar alloy, and
...
The number of signals per second above a threshold without a light source is known as the dark count rate.  At ambient temperatures they
...
threshold it can be tuned - however, because
for a DM search we would like as low of a threshold as possible, choosing PMTs with low dark count rates is essential.
...
The second process is a more subtle but often continuous stream of light.  Known as ``micro light emission" it is considerably harder to
...
Directly following the pulse of a PMT hit a secondary pulse may occur.  Known as afterpulses, they can occur within 10s of nanoseconds to
...
widths of several
microseconds.  Because they are an inevitable side effect of PMTs it is important to carefully characterizing them.
...
specifically
charge to mass ratio - can be calculated.  $\delta t_{\mathrm{ap}}$ ranges from several hundred nanoseconds to several
...
Because it is not possible to eliminate all residual gas every PMT suffers from some ionization-based afterpulsing.  A better vacuum
...
transit time spread (TTS).  Because an many PMTs observe an event the TTS quantifies how close together there signals should be.  Thus
...
been used in a number of LXe TPCs including XENON100, and an electronic recoil measurement at
...
(\figref{fig:xenon1t_tpc_tpc}).  The interior of the wall consists of 24
...
with a 1360 mm range extend from the bottom PMT array to above the diving bell and are used for filling and recovery.  Placed around the
field cage are cable trays that power and transfer signals from the PMTs and temperature sensors.  They are made of PTFE and the cables
are held in place by PTFE spacers (\textbf{check if spacers is right word}).
...
using a Helicoflex seal.  Surrounding it is the 2490 mm tall by 1620 mm diamater outer cryostat separated by vacuum.  It is also
...
A total of 3.5 tons of xenon is stored in the cryostat.  In addition to the 2.0 t inside the TPC, extra 1.5 t is LXe between the
TPC and inner vessel.  20-30 kg make up the GXe.  The nominal LXe temperature is $T_{0} = -96^{\circ}\ \mathrm{C}$.  Gas near the top of
...
To achieve this purity we use a Kr distillation column, seen in \figref{fig:xenon1t_kr_dist_column}.  The column is has a total
...
LXe dark matter experiment.  Measurements are done using a rare gas mass spectrometer (RGMS) that uses cryogenic gas
...
(\citeref{Aprile2017d}).  It operates using the reverse principle, i.e. Rn-enriched xenon is coalesces near the bottom of the column and
...
($z = 0\ \mathrm{m}$) and highest ($z = 10\ \mathrm{m}$) rings each contains 24, while the three between them have 12.  The quantum
efficiency
...
$g_1$, $g_2$, electronic and nuclear recoil bands, and many more.  Depending on the source they can be performed inside the TPC (internal)
...
calibrations (\secref{subsubsec:xenon1t_calibrations_external}), where events are focused in just one section of the TPC near the
...
is a worry that
$\mathrm{C H_3 T}$ adheres to the cryostat, which would prohibit total removal.  For this reason a $\mathrm{C H_3 T}$ calibration has not
...
(\secref{subsec:det_char_position_reconstruction}).  PMTs that are white near the cross are turned off.  S2s are marked by
green circles in the middle panel.  After the primary S2 two smaller ($< 40\ \mathrm{PE}$) S2s follow that are
$\lesssim 3\ e^-$.}
...
$\mathrm{^{83m}Kr}$, \ce{^{222}Rn}, \ce{^{137}Cs}, \ce{^{228}Th}, americium beryllium, a neutron generator, and LEDs have been used for
calibration.
...
analyses.  \figref{fig:calibrations_kr_s1_s2} shows corrected and uncorrected S1s and $\mathrm{S2_b s}$ for \metakr events.  While the
...
event.  This is typically done with an LED inside the TPC.  Blue light is used since LEDs in our energy do not exist, which despite
...
under-amplified electron avalanch.  This is generally the most challenging aspect to model.
...
impurities in the LXe, so longer traversed distances lead to fewer photons (\secref{subsec:importance_procedure_effects_photons}).
...
simulations confirm this, especially when a few cm away from the wall - differences in
...
detector for calibration and removing when finished meant background data could resume immediately.  The most common source was
\ce{^{137}Cs} because the mean free path of its 661.7 keV \gammaray (${\sim}5\ \mathrm{cm}$) allowed it to reach the fiducial
volumes.  For
XENON1T and other ton-scale detectors to come it is less effective, as an early attempt shortly after the detector came online.  Using
...
\caption{Change figure to include SR0.  Electron lifetime over SR1.  Markers represent measurements using
$^{83\mathrm{m}}\mathrm{Kr}$.  The green line
and shaded region represent the median and credible region of the best-fit model.  A hotspot in June triggered a gate washing, where the
gate was briefly lifted out of the LXe.  It is suspected the LXe came in contact with parts of the TPC previously only in gas, releasing
impurities into the liquid.  The model is derived using \ce{^{222}Rn}, \ce{^{220}Rn}, and \ce{^{218}Po} \alphadecays and is
...
look at the
smallest features in the waveforms of events that have the width of an S2, and therefore must be a low integer of $e^-$.  These can
result
...
cS2 are the corrected S1 and S2.  An event of the same energy and type at the same electric field will result in identical cS1s and
cS2s.  For XENON1T the top PMT array for S2s is ignored.  To differentiate from the total S2 \stwob and \cstwob are used to represent the
portion
...
the fundamental limit of the resolution.  We can see our energy resolution is better than XENON100, PandaX, and in some places LUX.
...
Light and charge yields are the number of photons and electrons produced from a recoil.  They are fundamental processes independent of
...
standard deviations.  The SR1 reconstruction bias results are shown in \figref{fig:det_char_bias_smearing_s1_s1}.
...
state and be pulled outside the FV.}
...
free path that it is possible they pass through the water shield (\secref{subsec:xenon1t_water_shield}).  However, simulations show that
...
Detector materials were screen before purchasing (\secref{subsec:xenon1t_pmts, xenon1t_cryo}) to ensure
...
first is we want to maximize our FV.  But because including these events can affect on our sensitivity we need to know
what safe distances from the top, bottom, and wall are.  The second is once we choose the FV it is important to have a reliable estimate
...
To adress the first two points we model our expected contamination with events whose positions are reconstructed outside the
...
Three complementary S2$_{\mathrm{b}} \mdash r$ PDFs were
...
KDE for a 5-dimensional predictor of the event distribution inside the TPC.
...
\begin{figure}
\centering
\includegraphics[width=\textwidth]{wall_charging_radius}
...
Accidental coincidence does not benefit from fiducialization like much of our other background.  In fact,
...
distributions for \ce{^{220}Rn}, $\mathrm{^{241}AmBe}$, and the NG each were included in the electronic and nuclear recoil band fitting
...
SR0 and SR1 and NG is performed in SR1.
...
\figref{fig:er_nr_calibrations_wimp_contours} shows the background model with contours for 50 and 200 GeV WIMPs overlaid from First
Results.  The ER band is clearly
present and the overlap between the two reinforces the necessity of lowering the overall background to improve sensitivity.
...
The conversion of truth MC to cS1 and \cstwob is referred to as a ``fast MC'' (a random number is drawn from a uniform distribution to
select a set of parameters).  ``Truth MC'' will refer to the simulated events before the fast MC ($E_{\mathrm{true}}$,
...
to vary freely with $A,B > 0$.  The number of electron-ion pairs that recombine is
...
interactions are clustered near a particular $\phi$ that is closest to the calibration source.  Because we expect our
detector does not vary along $\phi$ the results of the lopsided position distribution are applied to the entire active volume.
...
distribution more complicated.  The number of ions is
electronic recoils
...
waveforms are generated with DAQ conditions that have been measured to replicate real waveforms as well as possible.  Although its
performance is difficult to measure, it is expected to be reasonably accurate.
...
the bias $\mu_{b, \mathrm{S1}}$ and standard deviation the smearing $\sigma_{b, \mathrm{S1}}$ for each bin.  They are shown in
the left panel of
...
\begin{equation}
G (x, y) = \frac{g_{2\mathrm{b}} \mathcal{C}_{\mathrm{S2_b}}(x_{\mathrm{rec}}, y_{\mathrm{rec}})}{\eta} =
...
\noindent where $\sigma_G$ is the gas gain resolution and is 0.24 and 0.25 for SR0 and SR1.  As with the S1 there will be some bias and
smearing, though to a lesser degree.  Using the same assumptions we find
...
difference is too small to impact the results and can be disregarded.  $\mu_{b, \mathrm{S2_b}}$ and $\sigma_{b, \mathrm{S2_b}}$ are shown in
...
Each event must pass a series of cuts.  There are only a couple that can be applied
...
as a result of each.  The combined cut acceptance is applied in the fast-MC for consistency, and are shown in
...
With the parameters and procedure outlined in \secref{sec:er_nr_calibrations} the results are now
presented.  The posterior of the MCMC fit is here defined as the final $1000\ \mathrm{iterations}\ \times 200\ \mathrm{walkers}$.  The
...
${\sim}2$ as shown in \figref{fig:er_nr_calibrations_results_gr}.  Ideally it would be closer to 1 but time restrictions did not allow
continued fitting.  Regardless, $\hat{R} {\sim} 2$ indicates
the walkers are close to the best-fit PDF, though convergence can never be proved.
...
$n_{\mathrm{ex}} / n_{\mathrm{ion}}$ are listed.  For each variable the initial values or allowed range, prior distribution, and
posterior are given.
...
The initial or permitted values, prior distributions, and posteriors are shown in \tabref{tab:er_nr_calibrations_results_er}.  Because we
do not use asymmetric gaussian distributions the uncertainty on the NR microphysics parameters differ slightly from the values
...
\hline
\hline
Parameter & Prior & Prior Distribution & Source & Posterior & Comment \\
\hline
...
$E_0$ &  & Free &  & \ce{^{220}Rn} & Recombination \\
$E_1$ &  & Free &  & \ce{^{220}Rn} & Recombination \\
$\gamma_{\mathrm{er}}$ &  & Free &  & \ce{^{220}Rn} & Recombination \\
$\omega_{\mathrm{er}}$ &  & Free &  & \ce{^{220}Rn} & Recombination \\
$\delta_{\mathrm{er}}$ &  & Free &  & \ce{^{220}Rn} & Energy threshold \\
...
\begin{figure}
\centering
\includegraphics[width=0.8\textwidth]{ERNRFitRunCombo_180420_ng_sr1_cs2_2_col_components_log}
\caption{Result of fit for NG in $\mathrm{cS2_b}$.}
\label{fig:er_nr_calibrations_results_ng_cs2}
\end{figure}
...
acceptances are applied.  Parameters are shared within science runs and given in
\tabref{tab:er_nr_calibrations_parameter_determ_mc_match}.
...
$\sigma_{G, \mathrm{SR0}}$ & $0.240\ \mathrm{PE}$ & Fixed & $-$ & SR0 gas gain resolution \\
$\sigma_{G, \mathrm{SR1}}$ & $0.250\ \mathrm{PE}$ & Fixed & $-$ & SR1 gas gain resolution \\
...
$\sigma_{e}$ & $0 \pm 0.02$ & Normal &  & $\sigma_{e, \mathrm{SR0}} = 2\sigma_{e, \mathrm{SR1}}$ \\
S2 Threshold & 200 PE & Fixed & $-$ & Requirement \\
...
\hline
\hline
Parameter & Prior & Posterior & Comment \\
\hline
\multicolumn{5}{c}{\ce{^{220}Rn}} \\ \cline{2-4}
\hline
\ce{^{220}Rn} & $\geq 0$ & & \\
AC \ce{^{220}Rn} & $\mathrm{Norm}(\mu = 5.94, \sigma = 1.19)$ &  & \\
\hline
\multicolumn{5}{c}{\ce{^{241}AmBe}} \\ \cline{2-4}
\hline
\ce{^{241}AmBe} & $\geq 0$ & & \\
AC \ce{^{241}AmBe} & $\mathrm{Norm}(1.6, \sigma = 0.16)$ & & \\
ER in \ce{^{241}AmBe} & $\geq 0$ & & \\
\hline
\multicolumn{5}{c}{NG} \\ \cline{2-4}
\hline
NG & $\geq 0$ & & \\
AC NG & $\mathrm{Norm}(\mu = 7.94, \sigma = 0.79)$ & & \\
ER in NG & $\geq 0$ & & \\
\hline
\hline
\end{tabular}
\caption{Accidental coincidence and electronic recoil background distributions for \ce{^{220}Rn}, \ce{^{241}AmBe}, and NG
calibrations.  AC is tightly constrained via the method outlined in
...
it is difficult in some cases to determine if in fact scatters occurred - particularly at low energy.  The fast-MC
...
\figref{fig:dark_matter_results_selection_er_nr}.  The yellow band denotes the acceptance from the S2 threshold
($\mathrm{S2} > 200\ \mathrm{PE}$) condition so is only relevant at lower energies.  The efficiency of
the professor in identifying an S1, represented in pink, is also only impactful at low energy.  The S1 and S2 cut acceptances are shown in
red and green, respectively.  They
...
\includegraphics[width=\textwidth]{}
\caption{}
\label{fig:dark_matter_results_selection_er_nr}
\end{figure}

Our energy ROI is selected by requiring cS1 threshold $3 \leq \mathrm{cS1} \leq 70\ \mathrm{PE}$, corresponding to roughly
$[1.4, 10.6]\ \mathrm{keV_{ee}}$ and $[4.9, 40.9]\ \mathrm{keV_{nr}}$.  The total acceptance is the product of the processor, S2
threshold, cS1 threshold, and
S1 + S2 cut acceptances.  Events that are gained or lost at the fiducial volume edge but such effects are
