...
\caption{Positions of background events for SR1.  The 1 tonne fiducial mass used in the SR0-only analysis is defined by
...
removed before analysis.  Known as ``blinding'', this prevents bias that might occur from adjusting analyses to better fit or reject
these events.  Data selection includes a valid (e.g. $0 < t_d < t_{d,\mathrm{max}}$) S1 and S2 with $\geq 3$ PMTs observing the S1 in
$< 100\ \mathrm{ns}$.  A position
...
results from our loss in acceptance at low mass (\figref{fig:dark_matter_results_acceptances}).  The rise at
