...
calibrations, where events are focused in just one section of the TPC near the wall.  The calibration source box is installed in the
...
getters.  \figref{fig:xenon1t_pur_cal_box_schematic} shows the schematic for the source box.  When the valves are opened,
...
External calibrations are performed with the source outside the cryostat.  In XENON100 this was done by inserting a source inside the lead
...
\caption{Energy spectrum of the total nuclear recoil background in the 1 t FV.  Image credit: \citeref{Aprile2016a}.}
...
rates.  While the rate of events throughout the entire volume of the TPC was consistent, a radius-dependent rate was discovered.  This is
shown in \figref{fig:backgrounds_wall_charge}, where events reconstructed inside the designated radius (29, 32, 36.94, and 39 cm) are
plotted from February to September in SR1.  For $r < 29\ \mathrm{cm}$ the limited statistics prevented precise measurements.

The decay rates $R$ show an rise over time, with larger $r$ corresponding to greater increase.  One possibility might be a leak - for
...
short half-lifes of \ce{^{222}Rn} and \ce{^{218}Po} mandate one that is growing, which is in disagreement.  Besides, a leak should enlarge
the total decay rate, which is not observed.
...
A second explanation is charge buildup on the PTFE.  Drifting \electron from events near the wall attaching to the PTFE will slowly alter
the electric field, primarily in the $\hat{r}$ direction.  The cumulative decay rate would not be impacted, and events will move
further inward as this component becomes stronger.  Furthermore, the rate of increase should grow with radius as the E-field distortion
becomes more significant until it reaches the point where it is losing more events than gaining.  Because 39 cm is too far for latter,
\figref{fig:backgrounds_wall_charge} supports that conclusion - at least of the former - as the slope for each fit grows with $r$.  The
