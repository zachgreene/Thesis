...
$4\pi$ and is capable of emission rates as low as $10\ \mathrm{n\ s^{-1}}$ (to reduce pile-up) and as high as $10^{7}\ \mathrm{n\ s^{-1}}$
(\citeref{Lang2017}).
...
\begin{figure}
    ...
    \begin{subfigure}[t]{0.45\textwidth}
        \centering
        \includegraphics[width=0.45\textwidth]{Decay_Chain_of_Thorium-232}
    \end{subfigure}%
    \begin{subfigure}[t]{0.45\textwidth}
        \centering
        \includegraphics[width=0.45\textwidth]{Decay_Chain_of_Uranium-238}
    \end{subfigure}
...
\begin{figure}
\centering
\includegraphics[width=\textwidth]{MuonFluxOverDepth}
...
\secref{subsec:det_char_elifetime} \ce{^{220}Rn} is used for our ER band because of the \ce{^{212}Pb} \betadecay with Q-value
569.9 keV and its electronic shell properties allow an internal calibration.  For NR we use \ce{^{241}AmBe} and a neutron generator (NG),
...
possible by running on GPUs, which provide a $10^{2-3}$ times speed increase.
...
\subsection{\ce{^{220}Rn}}
\label{subsec:er_nr_calibrations_rn220}
The \ce{^{220}Rn} decay chain is shown in the left panel of \figref{fig:backgrounds_decay_chains}.
The fits are performed using a Markov Chain Monte Carlo (\citeref{Foreman-Mackey}).



\subsection{\ce{^{241}AmBe}}
\label{subsec:er_nr_calibrations_ambe}
...
scatters for each neutron.  The $\beta$ from \ce{^{220}Rn} should not scatter so this is fixed to 1.
...
(\secref{sec:det_char}).  Once the   Priors are included for each parameter.
