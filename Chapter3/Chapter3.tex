...
\noindent where $\delta t_{\mathrm{ap}}$ is the delay time, $m$ and $q$ are the ion's mass and charge, and $V_{0}$ and $L$ are the
potential
...
\caption{Afterpulse spectrum for PMT 21 (top array) from August 25, 2017 LED calibration.  A photoelectron may ionize atoms and molecules
between the photocathode and first dynode.  Generally this effect is small since the interior of a PMT is kept at vacuum.  This PMT
has a significant afterpulse contribution with signals from \ce{He^+}, \ce{CH_4^+}, \ce{N_2^+}, \ce{Ar^+}, and singly- and doubly-ionized
xenon (\ce{Xe^+} and \ce{Xe^{++}}).  The large amount of xenon present suggests there is most likely a leak.  The PMT can be checked
regularly to monitor changes in these peaks.}
...
is used by ReStoX (\secref{subsec:xenon1t_restox}).  The cooling power can regulated by adjusting the $\mathrm{LN_2}$ evaporation
rate.  In nominal operations the nitrogen tower is kept at ${\sim}90^{\circ}\mathrm{C}$.  In the event the PTRs
...
things.  The cold \ce{LN_2} tower allows this to be done much more quickly than the ${\sim}15\ \mathrm{minutes}$ that would be needed if
it was at room temperature.  Sensors and controllers monitoring such a power loss are powered with uninterruptible power supplies (UPSs)
to ensure a successful transition and operation of $\mathrm{LN_2}$.  As with the PTRs a heater is connected to stabilize the temperature.
...
temperatures to reach ${\sim} 150^{\circ}\mathrm{C}$, and in a worst-case scenario can rupture - as did in XENON100 - allowing air to leak
into the system.  While the purity of the xenon plummets the more worrisome aspect is the increase in \ce{^{85}Kr}.
...
bottles (Kr column occurs after the purification loops before the HALO).  Unfortunately it cannot be operated at the pressure required
for the TPC bell so has not been in use since shortly after the experiment came online.
...
$\mathcal{F}$.  This is the fraction of proposed stretch move update steps that are accepted by a walker (step \ref{itm:rand}).  There is
...
our analysis is the Gelman-Rubin statistic $\hat{R}$ (\citeref{Gelman1992}).  It uses the average variance of the individual chains and
variance
