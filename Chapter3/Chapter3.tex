...
$E_{\mathrm{thresh}}$ such that if $E < E_{\mathrm{thresh}},\ r = 0$ (\citeref{NEST2013})..  Recombination fluctuations are defined as
...
distributions for \ce{^{220}Rn}, $\mathrm{^{241}AmBe}$, and the NG each were included in the fit.
...
$c_0$ &  & Free &  & \ce{^{220}Rn} & Recombination \\
...
\caption{Best fit values along with 68\% confidence intervals for electronic and nuclear recoil microphysics models.  The models are
...
Processor Efficiency & $0 \mdash 1$ & Normal &  & \\
S1 Cut Acceptance & $0 \mdash 1$ & Normal &  & \\
S2 Cut Acceptance & $0 \mdash 1$ & Normal &  & \\
...
\tabref{tab:er_nr_calibrations_results_backgrounds}.

For each of the \ambe and NG
fits accidental coincidence and background electronic recoil events were included, as normal and free distributions, respectively
(\secref{subsubsec:er_nr_calibrations_parameter_determ_additional_components}).  In addition the single and multiple scatter distributions
could be separated to better understand how each impacts the final fit.  Note that the latter means neutrons that scattered more than once
but still passed the single scatter cut (those that did not were rejected).
