...
Limiting external radiation is essential to a dark matter search.  In XENON100 lead bricks were used for shielding, but with the expected
sensitivity increase in XENON1T a water shield is instead used.
...
While the majority of background events come from detector materials (wall, PMTs, etc.) and are stopped within the first few cm of LXe,
there is a small amount distributed roughly uniformly throughout the entirety of the detector.  With the exception of long-lived Xe
isotopes and excited metastable states (usually following a nuclear recoil calibration, \secref{}) these come other nobles gases of which
trace amounts are left after distillation.  And because they have the same electronic properties as xenon they are not filtered by the
getters.  It is essential to reduce these backgrounds to minimize the likelihood that an event is
mistaken for dark matter.

Low-energy decays are the most dangerous since they fall in our region of interest.  
