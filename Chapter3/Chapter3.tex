...
An MCMC is \textit{reversible} if it satisfies the detailed balance equation

\begin{equation}
\pi (\vectlett{x}^{\prime}) P(\vectlett{x}^{\prime}|\vectlett{x}) = \pi (\vectlett{x}) P(\vectlett{x}|\vectlett{x}^{\prime})
\end{equation}

\noindent where $\pi(.)$ is a \textit{stationary distribution}.
...
outlined below.  An important property of some Markov Chains - including the Affine-Invariant Ensemble Sampler - is they are
\textit{reversible}, or satisfy \textit{detailed balance} describe equations and what means.
