...
\begin{figure}
    ...
    \begin{subfigure}[t]{0.45\textwidth}
        \centering
        \includegraphics[width=\textwidth]{PMTBottomArray}
    \end{subfigure}
...
$c_{\mathrm{lce}}(x, y, z)$ - seen in \figref{calibrations_lce_lce} - quantifies these differences.  Such position-dependent variations
...
We must also account for positional variations in S2.  However, because all S2s happen in the GXe an $x \mdash y$ correction is sufficient.  For
...
An important consideration is how close to the true position do we get when we reconstruct.
...
($g2 = 11.55 \pm 0.01_{\mathrm{stat}} \pm 0.24_{\mathrm{sys}}\ \mathrm{PE/e^-}$).  Because $g1$ depends on the PMTs, $\eta$ on the anode-gate electric field, and $G_e$ on
...
Returning to \eqnref{eq:calibrations_s1_s2} we see after solving for $g1/\eta/g2$ we can reconstruct $E$ for any ER event.  The (ER)
...
is significant because shared detector parameters (e.g. $W$, $g1$, $g2$, LCE, etc.) are constrained to a single value
...
processes, converting $n_{\mathrm{ph}} \rightarrow \mathrm{S1}$ and $n_{\mathrm{e}} \rightarrow S2$.  This is in part why the
...
reconstruction resolution ($((x - x_{\mathrm{true}})^2 + (y - y_{\mathrm{true}})^2)^0.5$) is shown in
...
absorption of
scintillation due to impurities in LXe) that influence the amount of light that reaches the PMTs, denoted $c_{\mathrm{lce}}(x, y, z)$
...
fraction of incident photons that generate at least one PE $\eta_{\mathrm{p}}$.  As shown in \eqref{eq:xenon1t_pmts_dpe}
$\eta_{\mathrm{\mu}} / \eta_{\mathrm{p}} = 1 + p_{\mathrm{dpe}}$.  Accounting for both of these effects the number of photons that
generate $\geq 1\ \mathrm{PE}$ is shown in \eqref{eq:er_nr_calibrations_parameter_determ_det_phys_npe}.

\begin{equation}
n_{\mathrm{p}} = \mathrm{Binom} \bigg( n = n_{\mathrm{ph}},\ p = \frac{g1 c_{\mathrm{lce}}(x, y, z)}{1 + p_{\mathrm{dpe}}} \bigg)
...
n_{\mathrm{pe}} &= n_{\mathrm{p}} + n_{\mathrm{dpe}}
\end{aligned}
\end{equation}

\noindent where $n_{\mathrm{pe}}$ is the total number of photoelectrons.
...
the truth and compare with the post-processed value.  The bias and smearing for S1s is shown in the left panel of
\figref{fig:er_nr_calibrations_parameter_determ_det_phys_bias_smear}, where both are defined as
$(\mathrm{processed} - \mathrm{truth}) / \mathrm{truth}$.  We fit slices in S1 of the bias with a gaussian, taking the mean to be the bias
$\mu_b$ and standard deviation to be the smearing $\sigma_s$ for each bin.  This is applied in the fast MC via
\eqref{eq:er_nr_calibrations_parameter_determ_det_phys_bias_smear}.

\begin{equation}
\mathrm{S1} = n_{\mathrm{pe}} (1 + \mathrm{N}(\mu = \mu_b,\ \sigma^2 = \sigma_s^2))
\label{eq:er_nr_calibrations_parameter_determ_det_phys_bias_smear}
\end{equation}
...
\begin{equation}
\mathrm{cS1} = \frac{\mathrm{S1}}{c_{\mathrm{lce}}(x, y, z)}
\label{eq:er_nr_calibrations_parameter_determ_det_phys_cs1}
\end{equation}

\noindent to obtain the corrected S1.  In the case of multiple scatters the S1 will be corrected by a single $c_{\mathrm{lce}}(x, y, z)$
...
likely occurred in different regions of the detector.  The cS1 then is not fully representative of the total light yield.  To be
