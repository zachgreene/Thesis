...
\caption{Layout of the purification system.  Image credit: \citeref{Aprile2017b}.}
...
An MCMC runs for $T$ iterations or ``steps'', giving the total number of samples to be $k \times T$.  Each sample contributes its
...
walker's future state.  The array of $k$ walkers of $\vect{\theta}$ over $T$ iterations forms a ``chain''.

In general $\vect{\theta}$ may contain parameters that are necessary for the model but of little interest,
known as \textit{nuisance parameters}, $\vect{\theta}^{(N)}$.  It can often b the case that we wish to \textit{marginalize}, or integrate
over $\vect{\theta}^{(N)}$
\begin{equation}
...
a powerful tool that can be run in a reasonable timescale.  For this analysis we use an implementation by \citeref{Foreman2013} of
...
$S^{(1)} = \{\vect{\theta}_i}, \forall i = k/2, . . ., k\}$.
...
\noindent where $z$ is randomly drawn from a distribution $g(z)$.  Choosing
$g(z^{-1}) = z g(z)$ keeps \eqnref{eq:er_nr_calibrations_parameter_determ_mcmc_walker_update} symmetric - that is, invariant
under $\vect{\theta}_p \leftrightarrow \vect{\theta}_i(t)$.  Choosing the acceptance probability of the proposed stretch move update
step as
\begin{equation}
q = \mathrm{min} \bigg( 1, z^{n - 1} \frac{p(\vect{\theta}_p)}{p(\vect{\theta}_i (t))} \bigg)
\end{equation}

\noindent ensures the chain will fulfill detailed balance.  \citeref{Foreman2013} uses
\begin{equation}
g(z) \propto
\begin{cases}
\dfrac{1}{\sqrt{z}} & \mathrm{if}\ z \in \Big[ \frac{1}{a}, a \Big], \\
0 & \mathrm{otherwise}
\end{cases}
\end{equation}
...
\item \label{itm:rand} Generate a random number from a uniform distribution $r \in [0, 1]$.  If $r \leq q$ then
$\vect{\theta}_i(t + 1) = \vect{\theta}_p$, otherwise $\vect{\theta}_i(t + 1) = \vect{\theta}_i(t)$.
...
For each iteration $t$ parameters are in some state $\vect{\theta}_i$.  
...
$\vect{\theta}_{t+1}^{(i)} = \vect{\theta}_t^{(i)}$.
\item Let $\mathcal{F} \def \frac{g_p}{g_t}$ and set $\vect{\theta}_{t+1} = \vect{\theta}_p$ when
$\mathcal{F} \geq 1$ - otherwise move with probability $\mathcal{F}$.  If the proposed step is not taken simply set
$\vect{\theta}_i(t + 1) = \vect{\theta}_i(t)$.
...
\frac{\frac{\mathcal{L}(\vect{\theta}_p) p(\vect{\theta}_p)}{p(\vectlett{x})}}
{\frac{\mathcal{L}(\vect{\theta}_t) p(\vect{\theta}_t)}{p(\vect{\vectlett{x}})}} =
\frac{p(\vect{\theta}_p|\vectlett{x})}{p(\vect{\theta}_t|\vect{\vectlett{x}})}
...
``safeguard'' or WIMP-like component is added to the ER model and constrained by \ce{^{220}Rn} calibration data (\citeref{Priel2017}).
...
(\citeref{Feldman1998}).  The coverage was tested using a variety of values of nuisance parameters.  The 90\% confidence interval upper
limit for $\sigma_{SI}$ is shown in \figref{fig:dark_matter_results_limit}.  The inset shows the limits and sensitivity's for this result
and \citeref{LUX2017a, PandaXII2017} normalize to the median sensitivity of this result.  At $> 50\ \mathrm{GeV}$ we can see our median
