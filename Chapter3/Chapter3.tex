...
by a plateau in the electron lifetime (\secref{subsec:det_char_elifetime}, \chapref{4}).  However, the drop in September does not follow
...
$\mathcal{C}_{\mathrm{lce}}(x, y, z)$ - seen in \figref{calibrations_lce_lce} - quantifies these differences.  Such position-dependent variations
...
electron lifetime used for Science Run 1 is shown in \figref{fig:det_char_elifetime_evolution}.  The flatness in $\tau_e$ is not
...
\citeref{NEST2015} leaves $k$ (\eqnref{eq:er_nr_calibrations_parameter_determ_nr_lindhard}), $\eta$ and $\lambda$
...
(\eqnref{eq:er_nr_calibrations_parameter_determ_nr_recomb_sigma}) as free parameters to fit.  The results are shown in
...
The best fit results and 68\% confidence intervals are listed in \tabref{tab:er_nr_calibrations_parameter_determ_nr_nest}.
...
were performed at different times they were fit simultaneously.\footnote{In SR0 \ce{^{220}Rn} and \ambe were fit independently.}  This
...
(with $x, y, z$ calculated from the S2, \secref{subsec:det_char_position_reconstruction}) when in reality the various interactions have
...
$\mathcal{C}_{\mathrm{S2}}(x, y)$ is the S2 $x \mdash y$ correction map (\secref{subsec:det_char_s2_position_correction}).  Approximating
...
with detector size.  Still, its total contribution is comparatively small.  AC distributions for \ce{^{220}Rn},
$\mathrm{^{241}AmBe}$, and the NG each were included in the fit.
...
done for $r < 36.94\ \mathrm{cm}$ where wall leakage could be neglected, the results were compared at larger radii and $z$, and are
relevant for dark matter data.
...
The objective in Bayesian analysis is to have a posterior density of the parameters based on the data in Bayes' formula
...
The microphysics for electronic recoils discussed in \secref{subsubsec:er_nr_calibrations_parameter_determ_er} has eight free parameters
that govern both recombination and recombination fluctuations.  They, along with $W$, the Fano Factor, and exciton-to-ion ratio, are
listed in \tabref{tab:er_nr_calibrations_results_er}.  The initial value, prior constraint, and posterior are listed for each
parameter.
...
\begin{figure}
\centering
\includegraphics[width=\textwidth]{ERcS1}
\label{fig:er_nr_calibrations_results_er_cs1}
\end{figure}

\begin{figure}
\centering
\includegraphics[width=\textwidth]{NRcS1}
...
\begin{tabular}{cccccc}
\hline
Parameter & Value & Prior Distribution & Posterior & Note \\
\hline
$W$ & $13.7 \pm 0.2\ \mathrm{eV}$ & Normal &  & \citeref{Dahl2009} \\
$F$ & 0.059 & Fixed & $-$ & \citeref{Doke1976} & \\
$n_{\mathrm{ex}} / n_{\mathrm{ion}}$ & 0.06-0.2 & Uniform & \\
$c_0$ &  & Free &  & ER photon yield \\
$c_1$ &  & Free &  & ER photon yield \\
$c_2$ &  & Free &  & ER photon yield \\
$c_3$ &  & Free &  & ER photon yield \\
$c_4$ &  & Free &  & ER photon yield \\
$E_{\mathrm{thresh}}$ &  & Free &  & ER energy threshold \\
$A$ & $\geq 0$ & Free &  & ER RF \\
$B$ & $\geq 0$ & Free &  & ER RF \\
\hline
\end{tabular}
}
\caption{Best fit values along with 68\% confidence intervals from the \citeref{NEST2015} analysis to available nuclear recoil light and
charge yields.}
...
Parameter & Value & Prior Distribution & Posterior & Note \\
\hline
$\eta$ & $0.933$ & fixed & $-$ & extraction efficiency \\
$\sigma_G$ & $0.250$ & fixed & $-$ & gas gain resolution \\
$v_d$ & $0.134\ \mathrm{mm\ \mu s^{-1}}$ & fixed & $-$ & drift velocity \\
$f_{\mathrm{aft}}$ & $0.627$ & fixed & $-$ & \\
$F_{\mathrm{E}}$ & $82\ \mathrm{kV\ cm^{-1}}$ & fixed & $-$ & \\
$g_1$ & $\mu = 0.1424,\ \sigma = 0.0062\ \mathrm{PE / ph}$ & Normal &  & \\
$g_{2b}$ & $\mu = 11.44,\ \sigma = 0.2\ \mathrm{PE} / e^-$ & Normal &  & \\
$p_{\mathrm{dpe}}$ & $[0.18, 0.24]$ & uniform &  & \citeref{Faham2015} \\
Processor Efficiency & $[0, 1]$ & Normal &  & \\
S1 Cut Acceptance & $[0, 1]$ & Normal &  & \\
S2 Cut Acceptance & $[0, 1]$ & Normal &  & \\
Multiscatter Cut & $[0, 1]$ &  &  &  & \\
S1 Bias $\mu_{b}^{\mathrm{S1}}$ & $0-1$ & uniform &  & \eqnref{eq:er_nr_calibrations_parameter_determ_det_phys_s1_bias_smear} \\
S1 Smearing $\sigma_{b}^{\mathrm{S1}}$ & $0-1$ & uniform &  & \eqnref{eq:er_nr_calibrations_parameter_determ_det_phys_s1_bias_smear} \\
S2 Bias $\mu_{b}^{\mathrm{S2}}$ & $0-1$ & uniform &  & \eqnref{eq:er_nr_calibrations_parameter_determ_det_phys_s2_bias_smear} \\
S2 Smearing $\sigma_{b}^{\mathrm{S2}}$ & $0-1$ & uniform &  & \eqnref{er_nr_calibrations_parameter_determ_det_phys_s2_bias_smear} \\
$\sigma_{e}^{\mathrm{rec}}$ & $\mu = 0,\ \sigma = 0.02$ & norm &  & \\
