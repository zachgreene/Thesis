...
Photons from the interaction site are weighted by the light collection efficiency (LCE) at the reconstructed positions
(\secref{subsec:det_char_lce}).  Next we must account for the probability of double photoelectron emission (DPE)
(\secref{subsec:xenon1t_pmts}).  Recall that the probability of DPE for Hamamatsu R11410 PMTs was found to be
$p_{\mathrm{dpe}} = 0.225 \pm 0.01$ with values ranging from 0.18-0.24 collectively for the different PMT models (\citeref{Faham2015}).
If we account for the emission of two photoelectrons by
a single photon with probability $p_{\mathrm{dpe}} = 0.18-0.24$ (\citeref{Faham2015}) the
photon detection efficiency becomes $g1 = $.
