...
impurities in the LXe, so longer traversed distances lead to fewer photons.
...
\begin{equation}
 = \frac{g_{2\mathrm{b}}}{g_1} \frac{\mathrm{cS1}}{E_{er}} - \frac{g_{2 \mathrm{b}}}{W} - \frac{\mathrm{cS2_b}}{E_{er}}
\end{equation}
...
%        \centering
    ...
    of g_ and g$_{2\mathrm{b}}$ over the first eight months of SR1.  They are stable to within 0.4\% and 1.4\% ($\pm 1 \sigma$ credible
...
plotted from February to September in SR1.  For $r < 29\ \mathrm{cm}$ limited statistics prevented precise measurements.

The decay rates $R$ grow over time, with larger $r$ corresponding to greater increase.  One possibility might be a leak - for
which there is other evidence (\secref{subsubsec:backgrounds_electronic_krypton, subsec:electron_lifetime_model_outgassing}) - but the
short half-lifes of \ce{^{222}Rn} and \ce{^{218}Po} mandate the leak to be growing, which is in disagreement.  Additionally, a leak should
increase the decay rate across the entire detector, which is not observed.
...
becomes more significant, until it reaches a point where it is loses more events than gains.  39 cm should be too far from the wall for
...
decay rates differ because \ce{^{218}Po} maybe left in a charged state following the decay of \ce{^{222}Rn} and be pulled out of the
FV, and is evident when comparing the $z$-distributions.

\figref{fig:backgrounds_detector_materials_wall_charge_radius} shows the expected rate increase dependence across $r^2$ using the best-fit
values from \figref{fig:backgrounds_wall_charge}.  It reports the event rate increase for the fiducial volume radial cut.  Both figures
...
\caption{Simulated neutron spectrum in fusion region of the NG (red dashed line) and spectrum at pulse shape discriminator during
calibration (blue shaded region).  Deconvolution of data from calibration (black solid line) is also show.  Spectra are normalized by
