...
between all five, while $g1$, $g2$, etc. are shared within each science run) to converge to a single value.\footnote{for First Results
...
The calibrations were fit in the First Results FV ($-92.9 < z < -9\ \mathrm{cm}$, $r < 36.94\ \mathrm{cm}$
...
\noindent where $r = 0 \mdash 1$ and can be parameterized using the Thomas-Imel box model (\citeref{Thomas1987})

\begin{equation}
\langle r \rangle = \Big( 1 - \frac{\mathrm{ln}(1 + n_{\mathrm{ion}} \varsigma / 4)}{n_{\mathrm{ion}} \varsigma / 4} \Big)
,\ \varsigma = \gamma_{\mathrm{er}} e^{-E / \omega_{\mathrm{er}}} F_{\mathrm{E}}^{-\delta_{\mathrm{er}}}
\labels{eq:er_nr_calibrations_parameter_determ_ti}
\end{equation}

\noindent where $\varsigma$ is a more flexible variant of \eqref{eq:ti_recomb} with free parameters $\gamma_{\mathrm{er}}$,
$\omega_{\mathrm{er}}$, and $\delta_{\mathrm{er}}$.  However, in our fits we observed
abnormal behavior in our light and charge yields caused by inconsistencies at low energy.  To rectify this we applied a Fermi-Dirac
scaling to \eqref{eq:er_nr_calibrations_parameter_determ_ti}, ultimately giving

\begin{equation}
\langle r \rangle = \Big( 1 - \frac{\mathrm{ln}(1 + n_{\mathrm{ion}} \varsigma / 4)}{n_{\mathrm{ion}} \varsigma / 4} \Big)
\frac{1}{1 - e^{-(E - E_0) / E_1}}
\label{eq:er_nr_calibrations_parameter_determ_ti_fd}
\end{equation}

\noindent with $E_0, E_1 \geq 0$.

Recombination fluctuations are defined as $\Delta r = A(1 - e^{-E/B})$.  All seven parameters (five from
\eqref{eq:er_nr_calibrations_parameter_determ_ti_fd} and two from $\Delta r$) are allowed
to vary freely (with no penalty term in likelihood) though $0 \leq \gamma_{\mathrm{er}} \leq 0.5$ and $E_0,E_1,A,B > 0$.  The additional
...
The best fit results and 68\% credible intervals are listed in \tabref{tab:er_nr_calibrations_parameter_determ_nr_nest}.
...
$\sigma_{e, \mathrm{SR0}} = 0.04$ and $\sigma_{e, \mathrm{SR1}} = 0.5 \sigma_{e, \mathrm{SR0}}$, with the difference because we are more
confident in $\tau_e$ for SR1 (\figref{fig:det_char_elifetime_evolution}).  The true and reconstructed electron loss probabilities
are given in
...
fit.  \figref{fig:er_nr_calibrations_parameter_determ_ac} shows the AC distribution in cS1-cS2$_{\mathrm{b}}$ space with the region of
interest shown in dotted red (median) and black ($-2 \sigma$) lines.

\begin{figure}
\centering
\includegraphics[width=\textwidth]{AC_SR1}
\caption{Accidental coincidence distribution in cS1-cS2$_{\mathrm{b}}$ space for Science Run 1.  The top and left panels show the
expected events in cS1 or cS2$_{\mathrm{b}}$ for 1 t and 1.3 t FV.  The NR median and $-2 \sigma$ are plotted to highlight our ROI.}
...
adopted from \citref{NEST2013} (ER) and \citeref{NEST2015} (NR).}
...
$g_{2b}$ & $11.44 \pm 0.20\ \mathrm{PE} / e^-$ & Normal &  & \\
...
Finally we must consider accidental coincidence and electronic recoil backgrounds during calibrations (due to our $r < 36.94\ \mathrm{cm}$
...
\begin{figure}
\centering
\includegraphics[width=\textwidth]{exposure1}
\end{figure}
...
To select our energy ROI we require $3 \leq \mathrm{cS1} \leq 70\ \mathrm{PE}$, corresponding to roughly $[1.4, 10.6]\ \mathrm{keV_{ee}}$
and $[4.9, 40.9]\ \mathrm{keV_{nr}}$.
...
,
contributing a total of $7.7 \pm 1.3\ \mathrm{events\ t^{-1}\ yr^{-1}\ keV_{ee}^{-1}}$.
