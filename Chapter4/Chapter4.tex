...
opposed to GXe.  Paper or note on magnetic pump.  Why QDrives over KNF - exposure to air.  RGA of impurities.
...
(\secref{subsubsec:er_nr_calibrations_parameter_determ_mcmc}) this analysis uses a Markov Chain Monte Carlo (MCMC).  However, a
Differential Evolution Markov Chain (DEMC) is used instead of the modified Affine-Invariant
Ensemble Sampler.  The two are similar in that they are both random walk Monte Carlos
and proposal steps are generated from fellow chains.

MCMCs use $k$ walker
to explore the target distribution
over $T$ iterations.  Each walker $i \in k$ has parameters $vect{\theta}_i$ with proposed steps based on current positions of the
walkers, making MCMCs memoryless - that is, any there is no advantage to having knowledge of previous steps.  As with the Affine-Invariant
Ensemble Sampler DEMC is reversible and satisfies detailed balance.  The DEMC method used in this fit follows from
...
\item \label{itm:acc_frac_demc} Define the acceptance of proposed states across all chains for $\textcal{f} = k_{\mathrm{acc}}/k$ where
...
\item If $\textcal{f} < 0.2$ then $\gamma_0 = 0.9 \gamma_0$.

\item If $\textcal{f} > 0.31$ then $\gamma_0 = 1.1 \gamma_0$.

\item Otherwise $\gamma_0 = \gamma_0 (2\textcal{f}^{1/2})$.
\end{enumerate}
...
$\vect{\theta}_{\mathrm{R1}}, \vect{\theta}_{\mathrm{R2}}$ along $\vect{theta}_{\mathrm{R3}} - \vect{\theta}_i$ where
...
\begin{equation}
\begin{aligned}
\mathcal{L}_i &= \mathrm{exp} \bigg( -\frac{(\tau_{e, i}^{(\mathrm{meas})} -
\tau_{e, i}^{(\mathrm{pred})})^2}{2\sigma_{e, i}^{(\mathrm{meas})}} \bigg) \\
\mathcal{L} &= \prod_i \mathcal{L}_i \\
\mathrm{ln}\, \mathcal{L} &= -\sum_i \frac{(\tau_{e, i}^{(\mathrm{meas})} -
\tau_{e, i}^{(\mathrm{pred})})^2}{2\sigma_{e, i}^{(\mathrm{meas})}}
\end{aligned}
\end{equation}

\noindent where $\tau_{e, i}^{(\mathrm{meas})}$ and $tau_{e, i}^{(\mathrm{pred})}$ are the measured and predicted electron
