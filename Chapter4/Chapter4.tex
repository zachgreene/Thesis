...
(\secref{subsec:det_char_ces}) $z$-dependence it is clear the values are accurate.
...
The PTRs and \ce{LN_2} supply the cooling power to the system (\secref{subsec:xenon1t_cryo}).  This is balanced by heat from the GXe and
...
to be $\dot{Q}_{\mathrm{PTR}} = -\dot{Q}_{\mathrm{H}} = -260\ \mathrm{W}$.  \eqnref{eq:electron_lifetime_model_vap_and_cond_heat_cons} can
be rewritten as
...
\eqnref{eq:electron_lifetime_model_vap_and_cond_heat_cons} $\dot{Q}_{\mathrm{GXe}} \sim 140\ \mathrm{W}$.  When purification is resumed on
May 6th
...
\eqnref{eq:electron_lifetime_model_vap_and_cond_heat_gxe} leads to greater overestimation of condensation over time if not
corrected, although this can be difficult since the behavior of \qdh corrects for temperature variations that can vary
regularly.  \figref{fig:electron_lifetime_model_vap_and_cond_heater_all} shows \qdh for the entire course of the experiment.
...
not exceeded 1\%.  The \ce{LN_2} coldfinger is considerably more turbulent then the xenon.  It has been kept at $90^{\circ}\mathrm{C}$
since June 2016 by flushing \ce{GN_2} from the nitrogen storage tank.  This may be because the temperature is a balancing act of the
different heat transfer rates (\eqnref{eq:electron_lifetime_model_vap_and_cond_heat_cons} with
...
\caption{Relative temperature variations for temperature sensors stationed on the \ce{LN_2} coldfinger (blue), in the unpowered cooling
...
From \figref{fig:electron_lifetime_model_slow_control_pur} it seems reasonable to neglect temperature dependence in the model.  Without
...
\hline
\hline
Date & Origin & Region & Possible cause & Comments \\
\hline
...
January 18-February 1, 2017 & Earthquake & GXe + LXe & Yes & Many possible dips \\
June 5, 2017 & Gate washing & LXe & Liquid level change & Remove hotspot \\
February 21, 2018 & Power glitch & GXe & No & PTR power drop of ${\sim} 50\%$ \\
