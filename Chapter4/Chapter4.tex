...
$\lambda_{\mathrm{att}}$ is dependent on the absorption and scatter lengths, $\lambda_{\mathrm{abs}}$ and
...
scatter without energy loss.  This is covered in greater detail in \secref{subsec:importance_procedure_effects_photons}.The attenuation length can then be written
...
incorrect measurement of the scintillation spectrum.
...
corrected.  The left panel of \figref{fig:electron_lifetimes_measurement_kr_cs1_halflife} shows the cS1s for the two peaks for the
1 t FV.  From the
...
The large statistics provide precise $\tau_e$ measurements that are only rivaled by \ce{^{212}Bi}.  Unfortunately, unlike
\alphadecays there are no other regular electronic recoil $\tau_e$ measurements to corroborate these values, aside from
\meta{129}{Xe} and \meta{131}{Xe} following and \ambe or NG calibration.  However, by checking the combined energy spectrum
...
\caption{Electron lifetimes \meta{131}{Xe} (left) and \meta{129}{Xe} (right) for April 1-7, 2017 (same as in
\figref{fig:electron_lifetimes_measurement_gammas_cs1_cs2}).  The red lines mark the region of the fit - $70 < t_d < 710\ \mathrm{\mu s}$
and $100 < t_d < 710\ \mathrm{\mu s}$ for \meta{131}{Xe} and \meta{129}{Xe}, respectively.  The change in fit boundary is to omit
...
$r \geq 36.94\ \mathrm{cm}$ at $V_c = 8\ \mathrm{kV}$ (${\sim}80\ \mathrm{V\ cm^{-1}}$) using NEST (\citeref{NEST2013}).  Internal
