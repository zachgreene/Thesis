...
\noindent where $E_d$ is its thermal activation energy.  \eqnref{eq:electron_lifetime_model_outgassing_sources_diffusion_temp} explains
...
\eqnref{eq:electron_lifetime_model_outgassing_sources_small_t} to \eqnref{eq:electron_lifetime_model_outgassing_sources_large_t}.
...
\hline
\end{tabular}
\caption{Outgassing rates of \htwoo and \ce{CO_2} from 316L stainless steel after various treatments.  Significant reductions from
bakeouts can be seen.  Data is taken from
\citeref{Nuvolone1977}.}
...
LXe as shown in \figref{fig:electron_lifetime_model_outgassing_lxe_temp}.  Deviations are $< \lesssim num\%$.
...
For GXe the temperature is less stable and is plotted it \figref{fig:electron_lifetime_model_outgassing_gxe_temp}.
...
turned off the cathode, likely allowing liberation of \ce{O_2^-} and other bound electronegative impurities, shrinking
...
(\secref{subsubsec:electron_lifetime_model_outgassing_model}) is defined as the number of free impurities
...
(\secref{subsec:xenon1t_detector_slow_control}) and included in the fit.  This allows automatic integration of time-varying nuisance
...
\begin{figure}
\centering
\includegraphics[width=\textwidth]{purificationpid_v6.1}
