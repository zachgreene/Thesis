...
scatters for each neutron.  $\beta^-$ do not scatter so it is not necessary to track the number of scatters for \ce{^{220}Rn}.  These
...
Bias and smearing (\secref{subsec:det_char_bias_smearing}) characterize the difference between true and reconstructed area of the
...
in all five calibrations.  SR0 and SR1 \ce{^{220}Rn} AC are constrained by each other through
...
\secref{subsec:er_nr_calibrations_parameter_determ_er,
subsec:er_nr_calibrations_parameter_determ_nr,subsec:er_nr_calibrations_parameter_determ_det_phys,
subsec:er_nr_calibrations_parameter_determ_cuts}
and are listed in
...
S1 and S2 selection efficiencies remove some events (\secref{subsec:er_nr_calibrations_parameter_determ_cuts}).  Parameters are shared
within science runs and listed in \tabref{tab:er_nr_calibrations_parameter_determ_mc_match}.
...
described in \secref{subsec:er_nr_calibrations_parameter_determ_additional_components} tightly constrains the AC shape.  The AC
