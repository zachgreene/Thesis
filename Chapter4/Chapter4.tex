...
using the elements listed in \tabref{tab:electron_lifetimes_isotopes}.  An additional method of minimizing the width of \cstwob using the
...
Following an \ambe or neutron generator calibration $\mathrm{^{129}Xe}$ and $\mathrm{^{131}Xe}$ will be in excited nuclear states.  Their
...
\ce{^{218}Po} & $\alpha$ & $6.115\ \mathrm{MeV}$ & 3.1 m & Background (\ce{^{222}Rn} daughter) \\
...
\subsubsection{Single-Scatter}
\label{subsubsec:electron_lifetimes_measurement_ss}
In the very beginning of XENON1T relatively little was known about the detector, i.e. background, electron lifetimes were measured by selecting single-scatter data.
...
(\secref{subsec:det_char_ly_cy}), so the number $\gammas$ and escaped \electron would vary according to position.  Because $\alpha$
...
A quality check to the data acquisition (\secref{subsec:xenon1t_daq}) is applied to remove occasional issues with the readout.

\figref{fig:electron_lifetimes_measurement_alphas_s1} shows S1 dependence on drift time for \alphadecays after these cuts.
