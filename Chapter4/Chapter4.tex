...
using elements that were distributed in the xenon, since external \gammaray sources such as \ce{^{137}Cs} and
...
background decays had not yet been investigated, but \ce{^{222}Rn} and \ce{^{218}Po} would have had too few events in the top of the
...
electrons to recombine with their parent or nearby \ce{Xe^+}.  The S1s are significantly higher than electronic or nuclear recoils so are
...
bottom, and
sides of the TPC as seen in \figref{fig:xenon1t_tpc_efield}.  Changes in the field would produce different light and charge yields
...
The selected \ce{^{222}Rn} and \ce{^{218}Po} events are indicated by red and blue boxes, respectively.  Events with
...
The data is binned in drift time slices and an initial fit is performed on the 50th percentiles of the \stwob distributions from each
...
events were cut using the preliminary fit.  Both fits are plotted in each panel for comparison.  \ce{^{222}Rn} (left) and
\ce{^{218}Po} (right) have good agreement.
...
exist so \ce{^{222}Rn} and \ce{^{218}Po} were fit together.  This turned out to give slightly lower $\tau_e$ than separate fits.  Once
...
\ce{^{83}Rb} electron capture will produce \metakr with a 74.8\% branching ratio.  The delayed coincidence between the 32.2 and 9.4 keV
...
\begin{figure}
\centering
\includegraphics[width=0.8\textwidth]{kr_electron_lifetime_log}
...
$\mathrm{^{129m}Xe}$ and $\mathrm{^{131m}Xe}$ following \ambe or NG calibrations.  \secref{subsec:electron_lifetimes_rn222_vs_kr83m}
...
electronic recoil measurements from \meta{83m}{Kr}, the short $t_{1/2}$ limit them to the few weeks following an \ambe or NG
calibration.
...
efficiency (e.g. \meta{83}{Kr} delayed coincidence or high \alphadecay cS1).  Instead a number of cuts are required
to remove unwanted data before the fit.

To select the 163.9 and 236.2 keV peaks an S2 single scatter cut is used to remove much of the underlying background.  Additional data
quality
...
using a ``scaling function'' and applied to science data (\secref{subsec:electron_lifetimes_rn222_vs_kr83m}).
...
Impurities are removed in the purification system (\citeref{subsec:xenon1t_pur}) by two SAES PS4-MT50-R getters.  They operate between
...
\texttt{PUR\char`_FC201} & Flow measurement (bottom loop) & purple & \\
\texttt{PUR\char`_FC202} & Flow measurement (top loop) & purple & \\
\texttt{PUR\char`_FV217} & Valve (bottom loop) & $color$ & Bypass Getter 201 \\
\texttt{PUR\char`_FV224} & Valve (top loop) & $color$ & Bypass Getter 202 \\
\hline
\hline
\end{tabular}
}
...
The PTRs and \ce{LN_2} supply the cooling power to the system (\secref{subsec:xenon1t_cryo}) and are balanced by heat from the GXe and
...
XENON1T came online it was halted entirely, as shown in \figref{fig:electron_lifetime_model_vap_and_cond_no_flow}.  Once the system
...
the long-term behavior in $\dot{Q}_{\mathrm{H}}$ as shown in \figref{fig:electron_lifetime_model_vap_and_cond_ptr_decrease}.  As a result
\eqnref{eq:electron_lifetime_model_vap_and_cond_heat_gxe} leads to increasing overestimation of condensation over time if not
corrected, although this can be difficult since \qdh also corrects for temperature changes of the
GXe.  \figref{fig:electron_lifetime_model_vap_and_cond_heater_all} shows \qdh over the course of the experiment.

\begin{figure}
\centering
\includegraphics[width=\textwidth]{heater_all_online}
...
Five lines connect the GXe region of XENON1T to the purification system (\secref{subsec:xenon1t_pur}).  Three lines are attached to
...
40-hour bakeout and achieved $<4\%$ the \ce{H_2O} outgassing rate of Sample A ($150^{\circ}\mathrm{C}$), highlighting the importance of
...
1\%.  From \figref{fig:electron_lifetime_model_slow_control_pur} it seems reasonable to ignore temperature dependence in the model.

\begin{figure}
\centering
\includegraphics[width=\textwidth]{temp_stability}
\caption{Relative temperature variations for temperature sensors on the \ce{LN_2} coldfinger (blue), in the unpowered cooling
tower (black), and just below the LXe surface (pink).  The variation is defined as the $(T - T_{\mathrm{med}}) / T_{\mathrm{med}}$ where
$T_{\mathrm{med}}$ is the median temperature from all data.  The \ce{LN_2} coldfinger shows the most volatility.  The GXe changes by
...
The outgassing of the system should rapidly decrease in the beginning of the experiment continue to do so slowly as time goes on.  A
...
November 21, 2016 & Power Disruption & LXe & No & & \\
January 18-February 1, 2017 & Earthquake & GXe + LXe & Liquid level change & & Other possible causes \\
June 5, 2017 & Gate washing & LXe & Liquid level change & & Remove hotspot \\
February 21, 2018 & Power glitch & GXe & PTR power drop of ${\sim}50\%$ & & \\
February 24, 2018 & Blackout & GXe + LXe & PTR power$\rightarrow 0$ & & \\
March 5-7, 2018 & Gate washings & LXe & Liquid level change & & Remove hotspot \\
April 5-10, 2018 & Purification upgrade & LXe & Yes & & 1" OD upgrade \\
\hline
\hline
\end{tabular}
}
...
\textbf{November 21, 2016}  Between SR0 \ambe and \metakr calibrations and dark matter data taking a power disruption
...
\noindent where S is some impurity.  Because impurities in this fit are modeled in \ce{O_2}-equivalence the attachment rate is
...
background could be used the lifetimes of \ce{^{222}Rn}-\ce{^{218}Po} combined were computed.  Late it was realized that this combined
fit slightly underestimated the lifetime to $\tau_e$ was recalculated with separate fits for \ce{^{222}Rn} and \ce{^{218}Po}.  However,
data before SR0 was deleted so the lifetimes could not be computed.  A comparison between the two was used to adjust the
\ce{^{222}Rn}-\ce{^{218}Po} fits to align with expected values.
...
\begin{figure}
\centering
\includegraphics[width=0.8\textwidth]{kr_correction}
\caption{}
...
\begin{figure}
\centering
\includegraphics[width=0.8\textwidth]{kr_ces_z}
\caption{}
