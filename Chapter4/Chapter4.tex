...
\secref{subsubsec:er_nr_calibrations_parameter_determ_cuts}) is used to remove much of the underlying background.  Additional data quality
...
\subsection{\ce{^{222}Rn} vs. $\mathrm{^{83m}Kr}$}
...
flow controls \texttt{FC201} and \texttt{FC202} (purple), getters (green), and the HALO monitor (red) are highlighted.  The path of xenon
during ordinary operations is shown in red.}
...
While xenon returning from purification will add heat to the system, with a total mass of more than 3 t and
...
that were released from the \ce{^{220}Rn} calibration.  Each resulted in a total drop of $\lesssim 10\ \mathrm{\mu s}$.  The power glitch
on November 21, 2016 causes a sudden purity decrease (\secref{subsec:electron_lifetime_model_detector_effects_spikes}).}
...
suspension of flow through one of the two purification
branches led to a decrease in $\tau_e$ (\figref{}).  Once the upgrade was completed the flow increased to ${\sim}55\ \mathrm{slpm}$ with
...
(\secref{subsubsec:er_nr_calibrations_parameter_determ_mcmc}) this analysis uses a Markov Chain Monte Carlo (MCMC).  MCMCs use $k$ walkers
...
\subsection{$\mathrm{^{83m}Kr}$ Evolution}
...
\begin{figure}
\centering
\includegraphics[width=0.8\textwidth]{ces_z}
