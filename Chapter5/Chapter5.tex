...
the results (\secref{sec:elifetime_fit}).  Finally, purification for the next generation XENON experiment, XENONnT, is outlined
(\secref{sec:xenonnt}).
...
The mobilities for holes, \ce{TMSi^+}, $\mathrm{O_2^-}$, \ce{^{226}Th}, \ce{^{208}Tl}, and $\mathrm{Xe_2^+}$ in in LXe are listed in
...
The final hardware upgrade was in the middle of June when a magnetic pump \citeref{Brown2018} was installed in parallel with the
...
\ce{^{222}Rn} background by ${\sim}45\%$.  The upgrades are discussed in more detail in \secref{subsec:electron_lifetime_model_ops}.
