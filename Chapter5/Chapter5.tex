...
(\secref{subsubsec:backgrounds_electronic_radon}), since \betadecays at ${\lesssim}\, 20\ \mathrm{keV}$ are inside the region of
...
This chapter focuses on the effects of electronegative impurities (\secref{sec:importance_procedure_effects}), electron lifetime measurements
(\secref{sec:electron_lifetime_measurements}), and the derivation (\secref{sec:electron_lifetime_model}) and results of an electronegative
impurity evolution model (\secref{sec:elifetime_fit}).  Finally, purification for the next generation XENON experiment, XENONnT, is briefly
outlined (\secref{sec:xenonnt}).



\section{Effects of Electronegative Impurities}
\label{sec:importance_procedure_effects}
Drifting electrons attach to electronegative impurities, such as \ce{O_2} and \ce{N_2}, lowering or possibly eliminating the S2
(\eqnref{eq:er_nr_calibrations_parameter_determ_det_phys_prob_elifetime}).  This
...
calculations estimate $\lambda_{\mathrm{scatt}} \sim 29 \mdash 50\ \mathrm{cm}$ \citeref{Ishida1997, Seidel2002, Chepel1994, Braem1992}.

Higher levels of impurities shorten the absorption
...
\caption[Absorption coefficients for 1 ppm \ce{H_2O} vapor and \ce{O_2}.  The Xe scintillation
spectrum is overlaid for comparison.]{Absorption coefficients for 1 ppm \ce{H_2O} vapor (solid red) and \ce{O_2} (dashed green).  The Xe
scintillation spectrum is overlaid for comparison (dashed blue).  \ce{H_2O} impacts Xe scintillation considerably
...
\citeref{Yoshino1976} at $165\ \mathrm{K}$ (in addition, $\mu_0$ was found to be $1900$ \citeref{Yoshino1976} and
$2200\ \mathrm{cm^2\ V^{-1}\ s^{-1}}$ \citeref{Miller1968}
...
Maxwellian distribution).  The mobilities for holes, \ce{TMSi^+}, $\mathrm{O_2^-}$, \ce{^{226}Th}, \ce{^{208}Tl}, and $\mathrm{Xe_2^+}$ in
LXe are listed in \tabref{tab:importance_procedure_effects_charge_mobilities}.  They are ${\sim} 10^6$ times smaller than the electron
mobility.
...
are ${\sim} 10$ times larger than ions.  Summarized data is given in \citeref{Aprile2006a}.}
...
activation energy $E_{\mathrm{a}}$.  \figref{fig:importance_procedure_effects_charge_hole_mobility} shows the hole mobility in
...
As the electron cloud drifts \electron attach via
...
\noindent where molecule $XY$ is separated into components $X$ and $Y$.  \ce{O_2} has a dissociation energy (the energy needed to
separate the oxygen atoms) of 5.116 eV, and \ce{O^-} has an electron affinity (energy released in forming ion) of 1.461 eV.
...
\electron and electronegative $XY$.  Oxygen was studied in \citeref{Aleksandrov1993, Aleksandrov1981, Aleksandrov2009}.
\end{enumerate}
...
using elements distributed throughout the xenon, since external \gammaray sources such as \ce{^{137}Cs} and
surface events could not reach the FV.  This was done primarily with $\mathrm{^{83m}Kr}$, $\mathrm{^{129m}Xe}$, $\mathrm{^{131m}Xe}$,
\ce{^{212}Bi}, \ce{^{218}Po}, and \ce{^{222}Rn}.\footnote{Other lines such as the $39.6\ \mathrm{keV}$ of $\mathrm{^{129}Xe}$ and
$80.2\ \mathrm{keV}$ of $\mathrm{^{131}Xe}$ during nuclear recoil calibrations were used but the short half-lifes (0.97 and 0.48 ns,
respectively) made measurements difficult.}  Information for each element is listed in \tabref{tab:electron_lifetimes_isotopes}.  An
additional method of minimizing the \stwob width of the \ce{^{212}Pb} electronic recoil band to address a discrepancy between
...
S1 (S2) single scatter cuts required that the second largest S1 (S2) be ${<}\, 0.2$ of the first.  The purpose of these cuts was to
...
On a few occasions a \ce{^{137}Cs} was placed outside the detector.  Because the water and LXe self-shielding greatly reduced the fraction of
...
\subsection[$\alpha$-decays][$\alpha$-decays]{$\boldsymbol{\alpha}$-decays}
...
electrons to recombine with their parent or nearby \ce{Xe^+} atoms.  Their large energy and high recombination fraction create S1s that are
...
\citeref{Aprile2017f}, but also because inhomogeneities in the electric field are expected to be smallest in the center of the TPC as
seen in \figref{fig:xenon1t_tpc_efield}.  Changes in field produce different light and charge yields
...
disqualifies the last 21 seconds per run (each run is 1 hour).  Further details for each are given in \secref{subsec:xenon1t_daq}.
...
\caption[cS1 and $\alpha$-corrected cS1 dependence on $z$ for \ce{^{222}Rn} and \ce{^{218}Po} $\alpha$-decays.]{cS1 (left) and
...
here that events from \ce{^{222}Rn} and \ce{^{218}Po} cannot be distinguished from one another.  This is in general not a problem since they
...
lifetimes are calculated independently, which provides a cross-check.  Because the decay of \ce{^{222}Rn} can leave \ce{^{218}Po}
in a charged state, \ce{^{218}Po} has slightly fewer ($5 \mdash 10\%$) events.  Electron lifetimes are calculated using 48 hours of data in
...
events were cut from the result of the preliminary fit.  \ce{^{222}Rn} (left) and \ce{^{218}Po} (right) have good agreement.
...
lines correspond to \ce{^{222}Rn} and \ce{^{218}Po} fits, respectively, and are drawn in the drift time region that was used (see
...
for \ce{^{212}Bi} during \ce{^{220}Rn} calibrations.\footnote{\ce{^{220}Rn} and \ce{^{216}Po} are also
...
observation.  Even if this were not the case the distribution of $\alpha$-corrected cS1s of \ce{^{220}Rn} (6.405 MeV) and \ce{^{212}Bi}
...
\subsection[$\gamma$-rays][$\gamma$-rays]{$\boldsymbol{\gamma}$-rays}
...
\figref{fig:electron_lifetimes_evolution_no_model} shows a disagreement between electron lifetimes from $\alpha$-emitters and \metakr
at $\tau_e \gtrsim 400\ \mathrm{\mu s}$.  Because an inaccurate electron lifetime produces an under- or over-corrected
$\mathrm{cS2_b}$, thereby changing the probability of its origin (\figref{fig:dark_matter_results_bands}), it is imperative to be able to
reliably state which should be used for different recoil types and energies.

One possible solution to this discrepancy is field non-uniformity in the TPC (\figref{fig:xenon1t_tpc_efield}).  Because the change in
recombination
...
$dE/de^-$ require independent lifetime corrections.  LUX also observed a disparity between \metakr and \ce{^{222}Rn}
...
are shown throughout the TPC according to the Noble Element Simulation Technique (NEST) assuming azimuthal symmetry \citeref{NEST2013}.  The
...
small.\footnote{\figref{fig:electron_lifetimes_rn222_vs_kr83m_field_z} assumes a homogeneous event distribution throughout the
TPC.  Because \ce{^{60}Co} and \ce{^{208}Tl} are in detector materials their event distributions increase along $\hat{r}$, where
...
not well detailed.  The code can be found at \citeref{NEST2018}.}  A separate field-dependent effect is discussed in
...
atomic mass, electronegative particles (e.g. \ce{O_2}, \ce{CO}, etc.) are expected to be more densely distributed in the GXe.  To
...
heater heat transfer rates.  They are defined in \eqnref{eq:electron_lifetime_model_vap_and_cond_heat_cons} so that
...
\figref{fig:electron_lifetime_model_vap_and_cond_no_flow}.  Once the system stabilized
...
\eqnref{eq:electron_lifetime_model_vap_and_cond_heat_gxe} leads to increasing overestimation of condensation if not
...
applied.  \figref{fig:electron_lifetime_model_vap_and_cond_heater_all} shows the original (from slow control) and corrected \qdh over the
...
\figref{fig:pressure_purification_after_upgrade_option_a} is marked by the first arrow at ${\sim} 75$ slpm using the QDrive configuration at
...
The most recent hardware upgrade was in the middle of June when a magnetic pump \citeref{Brown2018} was installed in parallel with the
...
be applied to the XENONnT upgrade (\secref{sec:xenonnt}).
...
\hline
$I_{\mathrm{G}}^0$ & $\geq 0$ & ppb & $9524_{-6612}^{+11309}$ & Initial GXe impurity concentration \\
$I_{\mathrm{L}}^0$ & $\geq 0$ & ppb & $117_{-72}^{+75}$ & Initial LXe impurity concentration \\
$\epsilon_{\mathrm{vap}}$ & $0 \mdash 1$ & $-$ & $0.321_{-0.219}^{+0.299}$ & Probability of LXe impurity vaporization, \secref{subsec:electron_lifetime_model_vap_and_cond} \\
$\epsilon_{\mathrm{cond}}$ & $0 \mdash 1$ & $-$ & $0.995_{-0.021}^{+0.004}$ & Probability of GXe impurity condensation, \secref{subsec:electron_lifetime_model_vap_and_cond} \\
$T_0$ & 163-298 & K & $294_{-11}^{+3}$ & Condensation temperature offset, \secref{subsec:electron_lifetime_model_vap_and_cond} \\
\hline
\hline
\end{tabular}
}
...
The leak rate in all four periods falls close to the expected value from \eqnref{eq:electron_lifetime_model_outgassing_leak} of
$165\ \mathrm{ppb\ kg\ day^{-1}}$.
...
\hline
\multirow{4}{*}{$A_{\mathrm{G}}$} & \multirow{4}{*}{$\geq 0$} & \multirow{4}{*}{$\mathrm{ppb\ kg\ day^{-1}}$} & May 1, 2016 & $357_{-74}^{+68}$ & \multirow{4}{*}{\secref{subsubsec:electron_lifetime_model_outgassing_model}} \\
& & & February 1, 2017 & $26.0_{-11.6}^{+15.7}$ & \\
& & & April 10, 2018 & $10.2_{-6.43}^{+9.10}$ & \\
& & & June 12, 2018 & $3.76_{-2.56}^{+4.42}$ & \\
\hline
\multirow{4}{*}{$\beta_1$} & \multirow{4}{*}{$\geq 0$} & \multirow{4}{*}{$\mathrm{day^{-1}}$} & May 1, 2016 & $0.0016_{-0.0011}^{+0.0017}$ & \multirow{4}{*}{\secref{subsubsec:electron_lifetime_model_outgassing_model}} \\
& & & February 1, 2017 & $0.0042_{-0.0030}^{+0.0045}$ & \\
& & & April 10, 2018 & $0.0094_{-0.0066}^{+0.0086}$ & \\
& & & June 12, 2018 & $0.0055_{-0.0036}^{+0.0051}$ & \\
\hline
\multirow{4}{*}{$\beta_2$} & \multirow{4}{*}{} & \multirow{4}{*}{$\mathrm{day^{-1}}$} & May 1, 2016 & $0.0012_{-0.0008}^{+0.0010}$ & \multirow{4}{*}{\secref{subsubsec:electron_lifetime_model_outgassing_model}} \\
& & & February 1, 2017 & $0.16_{-0.13}^{+0.13}$ & \\
& & & April 10, 2018 & $-0.0038_{-0.0859}^{+0.0913}$ & \\
& & & June 12, 2018 & $-0.086_{-0.129}^{+0.117}$ & \\
\hline
\multirow{4}{*}{$A_{\mathrm{L}}$} & \multirow{4}{*}{$\geq 0$} & \multirow{4}{*}{$\mathrm{ppb\ kg\ day^{-1}}$} & May 1, 2016 & $27.7_{-20.7}^{+29.8}$ & \multirow{4}{*}{\secref{subsubsec:electron_lifetime_model_outgassing_model}} \\
...
\hline
Aug. 22-Sep. 2, 2016 & $0 \mdash 1$ & GXe & $0.80_{-0.24}^{+0.18}$ & GXe-only purification \\
Aug. 22-Sep. 2, 2016 & $0 \mdash 1$ & LXe & $0.58_{-0.22}^{+0.11}$ & GXe-only purification\\
May 8-18, 2018 & $0 \mdash 1$ & GXe & $0.38_{-0.28}^{+0.38}$ & \ce{LN_2} cooling \\
May 8-18, 2018 & $0 \mdash 1$ & LXe & $0.882_{-0.044}^{+0.033}$ & \ce{LN_2} cooling \\
May 29-June 5, 2018 & $\geq 0$ & GXe & $0.66_{-0.48}^{+0.76}$ & Circulation stopped \\
May 29-June 5, 2018 & $0 \mdash 1$ & LXe & $0.301_{-0.078}^{+0.084}$ & Circulation stopped \\
June 5-12, 2018 & $ \geq 0$ & GXe & $0.93_{-0.58}^{+0.89}$ & Magnetic pump installation \\
\hline
\hline
\end{tabular}
}
...
\hline
$\alpha_1$ & Nov. 26-Dec. 5, 2016 & $0 \mdash 1$ & $0.829_{-0.020}^{+0.024}$ & \\
$\alpha_2$ & Dec. 19-21, 2016 & $0 \mdash 1$ & $0.67_{-0.26}^{+0.24}$ & \ce{^{220}Rn} calibration \\
$\alpha_3$ & Oct. 25-30, 2017 & $0 \mdash 1$ & $0.978_{-0.020}^{+0.014}$ & \ce{^{220}Rn} calibration \\
\hline
\hline
\end{tabular}
}
...
\hline
June 14, 2016 & Power glitch & GXe & PTR power$\rightarrow 0$ & $65$ & $-30$ & $47.3_{-18.3}^{+21.4}$ \\
July 15, 2016 & \ce{LN_2} test & GXe & PTR power$\rightarrow 0$ & $225^{\ast}$ & $-100$ & $31.5_{-2.3}^{+2.2}$ \\
Nov. 21, 2016 & Power disruption & LXe & Temperature increase & $450$ & $-75$ & $2.92_{-0.27}^{+0.32}$ \\
Jan. 18-Feb. 1, 2017 & Earthquake & GXe + LXe & Liquid level change & $520^{\ast}$ & $-120$ & $2.75_{-0.19}^{+0.19}$ \\
June 5, 2017 & Gate washing & LXe & Liquid level change & $575$ & $-50$ & $1.05_{-0.15}^{+0.15}$ \\
Feb. 21, 2018 & Power glitch & GXe & PTR power drop of ${\sim}50\%$ & $600$ & $-150$ & $3.89_{-0.10}^{+0.10}$ \\
Feb. 24, 2018 & Blackout & GXe + LXe & PTR power$\rightarrow 0$, $F_{\mathrm{G}} = F_{\mathrm{L}} = 0$ & $500^{\ast}$ & $-150$ & $4.47_{-3.08}^{+2.66}$ \\
Mar. 5-7, 2018 & Gate washings & LXe & Liquid level change & $450$ & $-70$ & $1.54_{-1.16}^{+1.30}$ \\
\hline
\hline
\end{tabular}
}
...
cut the \ce{^{222}Rn} background by ${\sim}45\%$.  The upgrades are discussed in more detail in \secref{subsec:electron_lifetime_model_ops}.
